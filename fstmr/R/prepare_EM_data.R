prepare_EM_data <- function(data, params){
  ...
  if(params[['use_dist_mrf']]){
    data_inputs[['nn_list']] = nn_out[[1]]
    data_inputs[['nn_dist_list']] = nn_out[[2]]
  } else {
    data_inputs[['nn_list']] = nn_out
  }
...
get_nearest_neighbors <- function(locs1, loc_names, nn, nn_range, nn_range_lat, nn_range_lon, nn_range_time,
                                  ...
                                  remove_between_land = F, return_dists = F){
  ...
  if (return_dists){
    if (one_set_of_locations) {
      mku_result = make_graph_undirected(neighbors, dists = dists)
      neighbors = mku_result[[1]]
      dists = mku_result[[2]]
    }
    return(list(neighbors, dists))
  } else {
    if (one_set_of_locations) {
...
make_graph_undirected <- function(neighbors, dists = NULL){
  for (i in 1:length(neighbors)){
    for (j in 1:length(neighbors[[i]])){
      ...
      if (!(i %in% neighbors[[nbs[j]]])){
        ...
        if(!is.null(dists)){
          dists[[nbs[j]]] = c(dists[[nbs[j]]], dists[[i]][j])
        }
      }
    }
  }
