Estep <- function(model, params, data_inputs, parameters, call_from_IC){
  UseMethod('Estep')
}

Estep.one_var <- function(model, params, data_inputs, parameters, call_from_IC=F){
  ...
  no_cluster_variation = ifelse(MC_max == 1, T, !(max(apply(cluster_mat, 1 , var)) > 0))
  ...
  for (mc in 1:MC_max){
    ...
    if (mc == 1 || !all(cluster_mat[,mc] == cluster_mat[,mc - 1])){
      ...
      if (params[['seasonal_mean']]) {
        UTX = c_compute_UTX_single_for_season(data_inputs[['Phi_response_prof']], data_inputs[['response_profs']],
                                              parameters[['Omega_response']], parameters[['means_response']],
                                              cmi, as.numeric(parameters[['measurement_error_response']]), 
                                              as.double(data_inputs[['day']]))
        
        sse_mean <- c_compute_centered_obs_single_for_season(data_inputs[['Phi_response_prof']], data_inputs[['response_profs']],
                                                             cmi, parameters[['means_response']], as.numeric(parameters[['measurement_error_response']]),
                                                             params[['G']], as.double(data_inputs[['day']]))
      } else {
      ...
      # Compute relevant matrices/cholesky and sample
      E_step_list[['Sigma_inv']] <- E_step_list[['A_inv']] + E_step_list[['UTU']]
      
      # sample
      if(!params[['use_lanczos_training']]){
        ...
        cond_sample <- cond_mean + 
          Matrix::solve(E_step_list[['Sigma_inv_chol']], 
                        Matrix::solve(E_step_list[['Sigma_inv_chol']], rnorm(length(cond_mean)),
                                      system = 'Lt'), system = 'Pt')
        # compute likelihoods
        if (!no_cluster_variation | call_from_IC){
        ...
        cond_mean = fstmr:::c_CG(as(E_step_list[['Sigma_inv']], 'generalMatrix'), UTX)
        cond_sample = cond_mean + c_sample_lanczos(as(E_step_list[['Sigma_inv']], 'generalMatrix'),
                                                   rnorm(dim(E_step_list[['Sigma_inv']])[1]))
        
        # compute likelihoods
        
        if (!no_cluster_variation | call_from_IC){
          data_ll <- compute_likelihood_for_IS(sigma_chol = E_step_list[['Sigma_inv']],
                                               A_inv_chol = resp_scores_chol[['Linv_all']],
                                               me_response = parameters[['measurement_error_response']],
                                               response_profs = data_inputs[['response_profs']],
                                               cond_mean = cond_mean,
                                               UTX = UTX,
                                               sse_mean = sse_mean, 
                                               lanczos = params[['use_lanczos_training']])
        }
      }
  ...
  if(!no_cluster_variation | call_from_IC){
    ...
    like_ind <- rep(1, params[['MC_max']])
    like_spatial <- rep(1, params[['MC_max']])
  }
...
Estep.mult_var_ind <- function(model, params, data_inputs, parameters, call_from_IC=F){
  ...
  compute_conditional_distributions(response_prof = data_inputs$response_profs, 
                                    ...
                                    cond_probs =  p_mat,
                                    is_bgc = data_inputs$BGC,
                                    seasonal_mean = params[['seasonal_mean']], days = data_inputs[['day']],
...
Estep.mult_var_spat <- function(model, params, data_inputs, parameters, call_from_IC=F){
  ...
  for (mc in 1:MC_max){
    ...
    if (mc == 1 || !all(cluster_mat[,mc] == cluster_mat[,mc - 1])){
      ...
      if(!params[['use_lanczos_training']]){
        ...
        cond_mean = c_CG(as(E_step_list[['Sigma_inv']], 'generalMatrix'), UTX)
        cond_sample = cond_mean + c_sample_lanczos(as(E_step_list[['Sigma_inv']], 'generalMatrix'),
                                                   rnorm(dim(E_step_list[['Sigma_inv']])[1]))
  ...
  if(!no_cluster_variation | call_from_IC){
    ...
    wt = rep(1, MC_max)
    like_ind = rep(1, MC_max)
    like_spatial = rep(1, MC_max)
  }
