...
initialize.one_var <- function(model, params, data_inputs){
  ...
  if (G == 2 & params[['pc_response']] == 2) {
    ...
    p_seq_counter = 1
    pp_seq = cbind(1, p_seq)
    while (p_seq_counter <= params[['pc_response']]){
      p_seq_counter = p_seq_counter + 1
      pp_seq = cbind(pp_seq, p_seq^p_seq_counter)
    }
    pcs_initial = replicate(params[['G']], pp_seq[,1:params[['pc_response']]], simplify = F)
    ...
    #                                              p_seq^13, p_seq^14, p_seq^15, p_seq^16, p_seq^17)[,1:params[['pc_response']]], simplify = F)
  }
  ...
  var_guess <- sqrt(mean(sapply(1:n_profiles, 
                                function(x) {
                                  if (params[['seasonal_mean']]) {
                                    day = data_inputs[['day']][x]
                                    covariates <- c(1, sin(day/365.25 * 2 * pi * 1), cos(day/365.25 * 2 * pi * 1),
                                                    sin(day/365.25 * 2 * pi * 2), cos(day/365.25 * 2 * pi * 2),
                                                    sin(day/365.25 * 2 * pi * 3), cos(day/365.25 * 2 * pi * 3),
                                                    sin(day/365.25 * 2 * pi * 4), cos(day/365.25 * 2 * pi * 4),
                                                    sin(day/365.25 * 2 * pi * 5), cos(day/365.25 * 2 * pi * 5),
                                                    sin(day/365.25 * 2 * pi * 6), cos(day/365.25 * 2 * pi * 6))[1:ncol(parameters[['means_response']][[1]])]
                                    mean(as.double(data_inputs[['response_profs']][[x]] - 
                                                     Matrix::rowSums(data_inputs[['Phi_response_prof']][[x]] %*% 
                                                               parameters[['means_response']][[parameters[['cluster_mat']][x]]] %*% diag(covariates)))^2)
                                  } else {
                                    mean(as.double(data_inputs[['response_profs']][[x]] - 
  ...
  for (i in 1:data_inputs[['n_profiles']]){
    ...
    if (params[['seasonal_mean']]) {
      ...
      covariates <- c(1,sin(day/365.25 * 2 * pi * 1), cos(day/365.25 * 2 * pi * 1),
                      ...
                      sin(day/365.25 * 2 * pi * 5), cos(day/365.25 * 2 * pi * 5),
                      sin(day/365.25 * 2 * pi * 6), cos(day/365.25 * 2 * pi * 6))[1:ncol(parameters[['means_response']][[1]])]
      b = Matrix::rowSums(data_inputs[['Phi_response_prof']][[i]] %*% parameters[['means_response']][[cluster]] %*% 
                    diag(covariates))
    } else {
  ...
  for (i in 1:length(rp)){
    ...
    if (params[['seasonal_mean']]) {
      ...
      covariates <- c(1,sin(day/365.25 * 2 * pi * 1), cos(day/365.25 * 2 * pi * 1),
                      ...
                      sin(day/365.25 * 2 * pi * 5), cos(day/365.25 * 2 * pi * 5),
                      sin(day/365.25 * 2 * pi * 6), cos(day/365.25 * 2 * pi * 6))[1:ncol(parameters[['means_response']][[1]])]
      centered = rp[[i]] - phi_prof[[i]] %*% (Matrix::rowSums(parameters$means_response[[g]] %*% diag(covariates))- parameters$Omega_response[[g]] %*% pcs_array_response[i,1,])
    } else {
  ...
  vec_start = init_cov_params(params[['covariance_function']],
                              1, params=NULL,
                              nugget=params[['use_nugget']])
  
  # Remove marginal variance
  n_range_params = length(vec_start) - 1
  
  parameters[['range_params']] <- lapply(1:params[['G']], function(d) {
    matrix(nrow = n_range_params, ncol = params[['pc_response']],
           vec_start[2:length(vec_start)])
  })
...
initialize.mult_var_ind <- function(model, params, data_inputs){
  ...
  for (i in 1:data_inputs[['n_profiles']]){
    ...
    a = data_inputs[['response_profs']][[i]];
    if (params[['seasonal_mean']]) {
      total_basis <- 1 + 2 * params[['num_seasonal_basis']]
      covariates <- c(1,sin(data_inputs[['day']][i]/365.25 * 2 * pi * 1), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 1),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 2), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 2),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 3), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 3),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 4), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 4),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 5), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 5),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 6), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 6))
      b = data_inputs[['Phi_response_prof']][[i]] %*% Matrix::rowSums(parameters[['means_response']][[cluster]] %*% diag(covariates[1:total_basis]));
    } else {
      b = data_inputs[['Phi_response_prof']][[i]] %*% parameters[['means_response']][[cluster]];
    }
    c = data_inputs[['Phi_response_prof']][[i]] %*% parameters[['Omega_response']][[cluster]]
  ...
  for (i in 1:data_inputs[['n_profiles_TS']]) {
    ...
    if (params[['seasonal_mean']]) {
      ...
      covariates <- c(1,sin(data_inputs[['day']][i]/365.25 * 2 * pi * 1), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 1),
                      ...
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 5), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 5),
                      sin(data_inputs[['day']][i]/365.25 * 2 * pi * 6), cos(data_inputs[['day']][i]/365.25 * 2 * pi * 6))
      b = data_inputs[['Phi_predictors_prof']][[i]] %*% Matrix::rowSums(parameters[['means_predictors']][[cluster]] %*% diag(covariates[1:total_basis]));
    } else {
  ...
  parameters[['measurement_error_response']] = update_measurement_error(cluster_mat = parameters[['cluster_mat_BGC']],
                                                                        ...
                                                                        prof_ns = data_inputs[['profile_lengths_response']], 
                                                                        seasonal_mean = params[['seasonal_mean']],
                                                                        day = data_inputs[['day']][data_inputs[['BGC']]])
...
initialize.mult_var_spat <- function(model, params, data_inputs){
  ...
  parameters = fstmr:::initialize.mult_var_ind(model, params, data_inputs)
  
  for (i in 1:params[['EM_iter_init']]){
    MCEM_res = fstmr:::Estep.mult_var_ind(model, params, data_inputs, parameters)
    parameters =  fstmr:::Mstep.mult_var_ind(model, params, data_inputs, parameters, MCEM_res, i)
  }
  ...
  parameters[['Omega_response2']] = list()
  for(g in 1:params[['G']]){
    Sigma_nu = diag(parameters[['variances_response']][[g]]) - parameters[['Lambdas']][[g]] %*% diag(parameters[['variances_predictors']][[g]]) %*% t(parameters[['Lambdas']][[g]])
    eigen_nu = eigen(Sigma_nu, T)
    parameters[['Omega_response2']][[g]] = parameters[['Omega_response']][[g]] %*% eigen_nu$vectors
    parameters[['variances_response']][[g]] = eigen_nu$values
  }
  ...
  if (params[['pc_response2']] > ncol(parameters[['Omega_response']][[1]])) {
    ...
    parameters[['Omega_response2']] <- lapply(1:params[['G']], function(g) {
      cbind(parameters[['Omega_response2']][[g]], matrix(nrow = nrow(parameters[['Omega_response']][[1]]),
                                                   ncol = difference,
                                                   rnorm(difference * nrow(parameters[['Omega_response']][[1]]))))
    })
    parameters[['variances_response']] = lapply(1:params[['G']], function(g){
      c(parameters[['variances_response']][[g]], rep(min(parameters[['variances_response']][[g]]), difference))
    })
  }
  ...
  vec_start = fstmr:::init_cov_params(params[['covariance_function']],
                              1, params=NULL,
                              nugget=params[['use_nugget']])
  ...
  parameters$range_params_p <- lapply(1:params[['G']], function(d) {
    matrix(nrow = n_range_params, ncol = params[['pc_response1']], 
           vec_start[2:length(vec_start)])
  })
  
  parameters$range_params_r <- lapply(1:params[['G']], function(d) {
    matrix(nrow = n_range_params, ncol = params[['pc_response2']],
           vec_start[2:length(vec_start)])
  })
  ...
  # Avoid computational issues resulting from initialization
  for(g in 1:params[['G']]){
    ratios = parameters$variances_response[[g]] / parameters$variances_response[[g]][1]
    ind = which(ratios < 1e-3)
    if(length(ind) > 0){
      parameters$variances_response[[g]][ind] = parameters$variances_response[[g]][1] * 1e-3
    }
  }
...
initialize_old.mult_var_spat <- function(model, params, data_inputs){
  ...
  MCEM_res = fstmr:::Estep.mult_var_spat(model, params, data_inputs, parameters)
  
  pcs_mat = fstmr:::get_relevant_scores(MCEM_res$cond_mean, params$pc_predictors, params$pc_response1, data_inputs$n_profiles_TS)
  
  data_inputs[['response_profs']] = fstmr:::centered_response(cluster_mat = as.matrix(parameters[['cluster_mat']][data_inputs[['BGC']],]),
                                                              profs = data_inputs$response_profs,
                                                              Phi_prof = data_inputs$Phi_response_prof,
  ...
  data_inputs[['nn_list']] <- lapply(data_inputs[['nn_list']], function(x) {
    cumsum(data_inputs[['BGC']])[x[x %in% which(data_inputs[['BGC']])]]
  })
  
  parameters_temp = fstmr:::initialize.one_var(model, params, data_inputs)
  

  MC_max = params[['MC_max']]
  params[['MC_max']] = 1
  
  MCEM_res = fstmr:::Estep.one_var(model, params, data_inputs, parameters_temp)
  

  
  parameters_temp = fstmr:::Mstep.one_var(model, params, data_inputs, parameters_temp, MCEM_res, 1)
  
  params[['MC_max']] = MC_max
  
  parameters[['Omega_response2']] = parameters_temp[['Omega_response']]
  
  parameters[['lambda_lt']] = 10^4
