...
fstmr <- function(data, params, compute_diagnostics = F, verbose = F, printf=NULL){
  ...
  for (i in 1:params[['EM_iter']]){
    ...
    if(compute_diagnostics){
      diagnostics = update_diagnostics(model, i-1, parameters, params, diagnostics, Sys.time(),
                                       NULL, data_inputs)
    } 
    
    if(!is.null(printf) & class(printf) == 'function'){
      printf(model, diagnostics, parameters, data_inputs, params, i)
    }
...
init_params <- function(params, compute_diagnostics=F){
  ...
  if(is.null(params[['use_lanczos_training']])){
    params[['use_lanczos_training']] = FALSE
  }
  if(is.null(params[['use_lanczos_prediction']])){
    params[['use_lanczos_prediction']] = FALSE
  }
  if(is.null(params[['use_nugget']])){
    params[['use_nugget']] = FALSE
  }
  if(is.null(params[['start_params_previous']])){
    params[['start_params_previous']] = FALSE
  }
  if(is.null(params[['seasonal_mean']])){
    params[['seasonal_mean']] = FALSE
  }
  if(is.null(params[['use_dist_mrf']])){
    params[['use_dist_mrf']] = FALSE
  }
  if(is.null(params[['use_nugget']])){
    params[['use_nugget']] = FALSE
  }
  if(is.null(params[['skip_range_update_init']])){
    params[['skip_range_update_init']] = FALSE
  }
  return(params)
}
