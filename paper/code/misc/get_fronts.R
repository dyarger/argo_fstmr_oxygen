...
subantarctic_front <- data.frame(expand.grid('longitude' = long, 'latitude' = lat), 
  ...
  filter(latitude < -20) %>% 
  mutate(subantarctic = ifelse(longitude < (-72 + 360) & longitude > (-82 + 360) & latitude > -55, 
                               F, subantarctic)) %>% 
  mutate(subantarctic = ifelse(longitude < (175) & longitude > (140) & latitude > -52, 
                               F, subantarctic))
...
seaice_extent <- data.frame(expand.grid('longitude' = long, 'latitude' = c(seq(-75.5, -65.5, by = .25), lat)),
  ...
  mutate(seaice = ifelse(is.na(seaice), 0, seaice),
         seaice = seaice > .5) %>%
  mutate(seaice = ifelse(longitude < (-55 + 360) & longitude > (-90 + 360) & latitude > -60,
                         F, seaice))
...
all_fronts <- full_join(seaice_extent, subtropical_front) %>%
  ...
  mutate(South = ifelse(longitude < (60) & longitude > (0) & latitude > -40, 
                        F, South)) %>%
  mutate(South = ifelse(longitude > 360 & latitude > -40, 
                        F, South)) %>%
  mutate(South = ifelse(longitude > 105 & longitude < 150 & latitude > -40, 
                        F, South)) %>%
  mutate(South = ifelse(longitude > 140 & longitude < 165 & latitude > -45, 
                        F, South))
...
ggplot(data = all_fronts, aes(x = ifelse(longitude > 180, longitude - 360, longitude), 
  ...
  SO_theme  + 
  SO_coord +  continents + latitude_lines + longitude_lines
